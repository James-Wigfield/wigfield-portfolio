...
| `components/movers.tsx`                | The crew's bodies: seven more portlets whose feet, heading, speed and pose weights come from the Wordmark's choreography each frame (the shared `Stage`). A fade-and-grow through the portal's plane (own materials per runner), a walk whose stride follows distance actually covered, arms overhead for the carry, a squat to set down, a pat; every joint chases its target so phases blend. |
