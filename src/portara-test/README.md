...
| `components/workers.tsx`               | The portlets (`public/portara-test/portlet.glb`, the jointed robot from Claude Design) as autonomous upkeep. Every so often one materialises in the middle of the gate's opening, drops onto the plinth, hops down the steps, walks round the nearer END of the word (never through it, never through a post) to a letter picked at random and does a job on it with a tool built from primitives and held in its right hand: polishes it with a cloth, dusts it with a duster, inspects it with a magnifying glass, or sweeps round its foot with a broom, each with its own movement. Then it walks back, climbs the steps and dissolves into the portal, and there is a rest before the next. One out at a time, never the same letter or job twice running. They are 0.55 tall so they pass under the lowest ring. If the word starts its scroll flight, whoever is out dissolves where they stand. Own materials per worker for the fade; every joint chases its target so phases blend. The tool's "Show Portlets" box hides them. |
| `components/layout-tool.tsx` + `.css` | TEMPORARY. Click the wordmark or the gate (with its rings) in the 3D hero and move, turn or scale them with three.js transform handles, typed numbers or the keyboard; the wordmark also has a Curve control (degrees of arc, `,` and `.` keys). Show boxes hide the portlets and the ground grid. The layout is kept in localStorage (dropped when the code defaults change) and "Copy values" gives the block to paste over `LAYOUT` in hero-3d.tsx. Switch under the hero toggle. Once the layout is locked in: paste, then delete these two files and the lines marked LAYOUT TOOL in hero-3d.tsx. |
