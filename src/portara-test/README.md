...
Typecheck this folder's TypeScript (the portfolio has no tsconfig of its own):
`npx -y -p typescript@5.6.3 tsc -p tsconfig.portlets-check.json` from the repo
root. The one error it reports in `portal-ui/lib/workers.ts` is the mirror
file's `@portara/tenant-config` import, expected here (see below).
...
| `components/hero-3d.tsx`               | The "3D" hero (toggle under the header, choice kept in localStorage): the mark is the whole hero. The gate as a solid object from `public/portara-test/portara-hero.glb` (Claude Design export, see `temp/spinning-logo-handoff.md`) with the wordmark's three words as rings of letters orbiting at the handoff's radii and speeds, and PORTARA itself standing on the floor in front in 3D Jost type (0.55 cap height, 0.18 deep, 0.15em tracking, scaled 1.17), straight and turned to the camera's own heading so its baseline runs level across the screen (a Curve control in the tool can bend it round the gate instead), dropping in letter by letter after the gate rises. The camera eases in from a step further back over the first seconds. The portal carries an accent light in its opening that flares whenever a worker steps through. THROUGH THE PORTAL: once the word is down, scrolling on (12% to 42% of a viewport, `FLY`) lifts the letters in reading order and glides them back through the opening, chasing the scroll with a capped rate so a flick still plays as a glide; scrolling back brings them home. The rings are built from the font too, not the GLB: one radius (1.7) for all three, each word twice round its circumference, stacked by height (PORTALS 1.75, MCP 1.25, AGENTS 0.75). The composition (`LAYOUT` in the file): gate on the page's centre line turned 8 degrees to the camera, the word centred on the same line in front of it. Under 861px `LAYOUT_NARROW` is the same seen from further back. The copy is reduced to one h1 line ("Portara / Custom staff portals · Perth") and the meeting button along the foot of the scene; the pitch lives in the section below. Scrolling spins the rings up (boost decays over a few frames); letters take the accent as they cross the front; the camera stands a little above and drifts with the pointer; the gate rises and the rings spin in on load. Real cast shadows from the gate and every letter fall on the page's ground (shadow-only floor plane), the gate has a slight sheen and every hard edge drawn one step lighter so its corners read. No pinned jump in this version: the page scrolls straight on to the portal, which is in place below. The ring letters are rebuilt from the Jost SemiBold font at load (the GLB's traced outlines were faceted): `tools/subset-jost.mjs` turns `tools/jost-600-lab.ttf` (the Logo Lab's embedded subset, SIL OFL) into `public/portara-test/fonts/jost-600.typeface.json`, and each traced glyph is replaced in place with the same cap height, depth and baseline. The converter also removes overlapping contours (Jost draws P, R, A and E as stems and bowls laid over each other, which extruded put seams across the letter faces): those glyphs are unioned with polygon-clipping and emitted as fine polylines, the rest keep their curves. The wordmark's drawn lines are its face outlines only. react-three-fiber + drei, lazy-loaded. |
| `components/workers.tsx` + `components/portlets/` | The portlets (`public/portara-test/portlet.glb`, the jointed robot from Claude Design) as autonomous upkeep: up to THREE out at once, never appearing together, each with a letter reserved to it and a job of its own. One materialises in the middle of the opening, drops onto the plinth, hops down the steps, walks round the nearer END of the word to its letter, draws a tool, works, steps back to look, and walks home to dissolve into the portal. `workers.tsx` is only the React seam; everything else is in `portlets/`: `config.ts` (**`PORTLET_CONFIG`**, every tuning value), `manager.ts` (the pool; PACING as outings: the portal sits empty for a random quiet gap, then an outing of 1, 2 or 3 begins with its size drawn from weights, companions joining on a timer by chance, a third only when the other two are mid-job, nothing ever queued by a despawn; letter reservation with a neighbour gap and cooldown; exclusive jobs; the gate as a one-at-a-time lock; the ground under the feet RAYCAST against the gate model's own meshes), `steering.ts` (desired velocity -> capped acceleration -> velocity; facing with its own angular-velocity cap, eased in and out, arcing for anything under a near-reversal and stepping through a turn on the spot with weight shift and torso/head lead; feet and toes sampled against the steps so they step up before an edge and drop off it; gait phase from ground speed), `portlet.ts` (the state machine, pose blending with a 0.26 s crossfade on every state change, the ride-height spring, the tool solve), `contact.ts` (where a tool may touch a letter: the mesh raycast once in its own space and cached, with flatness, edge, clearance and bulge per point), `reach.ts` (where to stand so the tool's tip lands on the contact, from the tool's own tip vector), `actions.ts` (hammer a nail home, polish, paint with trips to a tall tin that is a reach target at its own standoff, the reload one eased sequence under hard caps on torso and head pitch, weld a seam with sparks, tighten a bolt with a socket wrench, vacuum), `tools/` (one procedural PBR tool per file, bevelled, with its tip offset and contact axis documented), `props.ts` (nail, bolt, can, shine, paint strips, weld bead, sparks, dust, pooled), `rig.ts` (the model's 27 meshes merged into 8 vertex-coloured draws per body). THE TOOL LIVES IN WORLD SPACE: solved from the contact point and normal with the tip's depth never negative, so it cannot enter a letter; the arm aims at the grip that falls out, and the body leans in for whatever the arm cannot cover. The layout panel's "Portlets" box hides them; "Contacts" draws every usable contact on the letters being worked, each tool's tip (green outside, red inside) and a readout of active portlets, held letters, draw calls, memory and tool-in-letter checks. In the console, `__portletManager.snapshot()` prints the pool, `__portletManager.groundProfile()` the raycast heights along the gate's centre, and `__portletView.shotBeside(slot)` returns a side-on photograph of one portlet as a PNG data URL. |
| `components/layout-tool.tsx` + `.css` | TEMPORARY. Click the wordmark or the gate (with its rings) in the 3D hero and move, turn or scale them with three.js transform handles, typed numbers or the keyboard; the wordmark also has a Curve control (degrees of arc, `,` and `.` keys). Show boxes hide the portlets, the rings of words orbiting the gate, and the ground grid, and turn on the portlets' contact debug (see workers.tsx). The layout is kept in localStorage (dropped when the code defaults change) and "Copy values" gives the block to paste over `LAYOUT` in hero-3d.tsx. Switch under the hero toggle. Once the layout is locked in: paste, then delete these two files and the lines marked LAYOUT TOOL in hero-3d.tsx. |
| `components/nav-reveal.ts`             | The header is not there at the top of the page (desktop): the hero has the whole viewport. The glass bar is pinned at the top but drawn only below the PORTARA letters' bottom edge (which the 3D hero publishes every frame), so nothing shows until the word, or once the letters have flown through the portal the gate's own foot, scrolls up into the header's band; then the bar is uncovered beneath it, its contents rising into place, and once it has gone the header is whole and stays for the rest of the scroll, with an accent hairline drawing across and fading, a shadow bloom and the logo, links and button settling in sequence. Scrolling back up covers it again. With no source (the 2D hero, loading) it slides down from above once the page has scrolled half a screen. Phones keep the header always. |
